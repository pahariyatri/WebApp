...
def index(request):
    ...
    thought = UserProfileInfo.objects.all()
...
def remove_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            return redirect("web:product", slug=slug)
    else:
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect("web:request-refund")
...
def details(request, pk_test):
    ev = Item.objects.get(id=pk_test)
    destination = get_object_or_404(Item, id=pk_test)
    destinations = get_object_or_404(Item, id=pk_test)
    dest = get_object_or_404(Item, id=pk_test)
